...
Original file is located at
    https://colab.research.google.com/drive/1BUirVsJLQEzGi7nlurY5tI-c_5nFGsDc
"""
...
def process_entries_with_duplicates(all_entries):
    ...
    processed_entries = []
    for _, group in title_groups.items():
        if not group:
            continue
        ...
        if duplicates:
            dup_outlets = [dup['media'] for dup in duplicates]
            if len(dup_outlets) == 1:
                media_string += f" (also ran in {dup_outlets[0]})"
            else:
                all_but_last = ", ".join(dup_outlets[:-1])
                media_string += f" (also ran in {all_but_last} and {dup_outlets[-1]})"
...
default_terms_text = ", ".join(DEFAULT_TERMS)
terms_text = st.text_area(
    "Search terms (comma-separated):",
    value=default_terms_text,
    height=90
)
...
when_choice = st.selectbox(
    ...
    index=0,
    help="pygooglenews `when` parameter"
)
